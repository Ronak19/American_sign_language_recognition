...
d = pwd;
if exist('svm_accuracy_log.csv')
    delete('svm_accuracy_log.csv');
end
fid = fopen('svm_accuracy_log.csv','a');
...
for i = 1 : length(ges)
    ...
    for j = 1 : num_of_groups
        ...
        if ~isempty(training_arr)
            SVMModel = fitcsvm(training_table(:,1:size(training_arr,2)-1),training_table(:,size(training_arr,2)),'KernelScale','auto','Standardize',true,'OutlierFraction',0.05);
            ...
            for k = 1 : size(testing_arr,1)
                if (predict(SVMModel,testing_arr(k,1:size(testing_arr,2)-1)) == testing_arr(k,size(testing_arr,2)))
                    if testing_arr(k,size(testing_arr,2)) == 1
